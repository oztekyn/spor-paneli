...
export default function AdminDashboard() {
  ...
  // 3. ÜYE EKLEME (FIXED: membership_type eklendi)
  async function handleAddMember(e: React.FormEvent) {
    e.preventDefault(); 
    if (!fullName) { alert("İsim alanı boş olamaz!"); return; }
    
    const { error } = await supabase.from('members').insert([{ 
      id: fullName.trim(), 
      slug: slugify(fullName), 
      password: newPass, 
      membership_type: 'Birebir', // Hatayı çözen kritik satır
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white p-4 md:p-8 font-sans selection:bg-green-500/30 overflow-x-hidden text-left">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-8 text-left">
        ...
        <section className="mb-12 bg-white/5 p-8 rounded-[3rem] border border-white/5 text-left shadow-2xl">
          <form onSubmit={handleAddMember} className="flex flex-col md:flex-row gap-4">
            ...
            <button type="submit" className="bg-green-500 text-black font-black px-12 py-4 rounded-2xl uppercase text-sm active:scale-95 transition-all shadow-xl">Üyeyi Kaydet</button>
          </form>
        </section>
              ...
              <div className="flex items-center gap-6 text-left">
                ...
                <div className="text-left">
                  ...
                  <div className="flex items-center gap-3 text-left text-left">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`${window.location.origin}/login`); alert("Giriş linki kopyalandı!"); }} className="text-gray-500 hover:text-blue-400 transition-colors"><Copy size={14}/></button>
                  </div>
                </div>
              </div>
        ...
        {selectedMember && (
          <div className="fixed inset-0 bg-black/95 z-[100] overflow-y-auto p-4 md:p-10 flex items-start justify-center">
            <div className="bg-[#080808] border border-white/10 rounded-[3.5rem] w-full max-w-6xl relative p-8 md:p-12 shadow-2xl flex flex-col my-auto text-left text-left">
                  ...
                  <div className="text-left w-full">
                    ...
                    <div className="flex flex-wrap gap-5 justify-start text-left">
                      <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3 text-left">
                        <Lock size={14} className="text-gray-500 text-left"/>
                        <span className="text-[10px] font-black uppercase text-gray-600 text-left">Giriş: {selectedMember.slug}</span>
              ...
              <div className="grid lg:grid-cols-2 gap-12 text-left">
                ...
                <div className="space-y-8 text-left text-left">
                  ...
                  {selectedDate && (
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6 animate-in slide-in-from-bottom duration-500 shadow-inner text-left text-left">
                        ...
                        <input type="time" value={trainingTime} onChange={(e) => setTrainingTime(e.target.value)} className="w-full bg-black border border-white/10 rounded-2xl px-6 py-4 text-xl font-black text-green-400 outline-none text-left text-left" /><textarea value={workoutProgram} onChange={(e) => setWorkoutProgram(e.target.value)} className="w-full bg-black border border-white/10 rounded-3xl p-6 h-40 text-sm outline-none resize-none text-left leading-relaxed" placeholder="Program..."></textarea><button onClick={saveAppointment} className="w-full bg-green-500 text-black font-black py-4 rounded-[1.5rem] uppercase text-[10px] tracking-widest active:scale-95 transition-all shadow-xl shadow-green-500/10">Kaydet</button>
                    </div>
                  )}
                ...
                <div className="space-y-8 text-left text-left text-left">
                    ...
                    <form onSubmit={addWeight} className="flex flex-col gap-4 bg-black/60 p-6 rounded-[2.5rem] border border-white/5 text-left text-left text-left"><p className="text-[10px] text-gray-600 font-black uppercase ml-2 tracking-widest text-left">Kilo Girişi</p><input type="number" step="0.1" value={newWeight} onChange={(e) => setNewWeight(e.target.value)} className="bg-transparent px-4 py-2 text-2xl font-black border-b border-white/5 outline-none text-left text-left" placeholder="00.0" /><button type="submit" className="bg-green-500 text-black font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest shadow-lg">Kilo Ekle</button></form>
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6 shadow-inner text-left text-left">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-left text-left">
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1 text-left">Omuz</p><input type="number" value={mShoulder} onChange={(e) => setMShoulder(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none text-left" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1 text-left">Göğüs</p><input type="number" value={mChest} onChange={(e) => setMChest(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none text-left" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1 text-left">Kol</p><input type="number" value={mArm} onChange={(e) => setMArm(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none text-left" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1 text-left">Bel</p><input type="number" value={mWaist} onChange={(e) => setMWaist(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none text-left" placeholder="0"/></div>
                      </div>
                      <button onClick={addMeasurement} className="w-full bg-blue-500 text-white font-black py-3 rounded-2xl uppercase text-[9px] active:scale-95 transition-all shadow-lg">Ölçüleri Kaydet</button>
                      {measurements.length > 0 && (
                        <div className="mt-4 border-t border-white/5 pt-4 space-y-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar text-left text-left text-left text-left">
                          {measurements.map((m, i) => (
                            <div key={i} className="flex justify-between items-center text-[9px] bg-black/40 p-2 rounded-lg border border-white/5 group text-left">
                              <span className="text-gray-500 text-left">{format(new Date(m.recorded_at), 'd MMM')}</span>
                              <span className="text-blue-400 font-black uppercase text-left">O:{m.shoulder} G:{m.chest} K:{m.arm} B:{m.waist}</span>
                              <button onClick={() => { if (window.confirm('Ölçüm silinsin mi?')) { supabase.from('body_measurements').delete().eq('id', m.id).then(() => showProfile(selectedMember)); } }} className="opacity-0 group-hover:opacity-100 text-red-500 transition-all text-left"><Trash2 size={10}/></button>
                            </div>
                          ))}
                        </div>
                      )}
                    </div>
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-4 shadow-inner text-left text-left">
                      <h3 className="text-[10px] font-black uppercase text-green-400 flex items-center gap-2 px-2 text-left text-left"><Apple size={14}/> Beslenme Yönetimi</h3>
                      <div className="grid grid-cols-2 gap-4 text-left text-left text-left">
                        ...
                        <textarea onBlur={(e) => saveDietPlan('snacks', e.target.value)} defaultValue={dietPlan?.snacks} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs outline-none h-20 resize-none italic text-left" placeholder="Snack"/>
                      </div>
                    </div>
                </div>
              </div>
