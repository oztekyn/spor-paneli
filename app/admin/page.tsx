...
import { Clock, Dumbbell, Trash2, Plus, Minus, CheckCircle, TrendingDown, AlertTriangle, XCircle, Copy, ChevronLeft, ChevronRight, Apple } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function handleAddMember(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('members').insert([{ id: fullName, slug: slugify(fullName), remaining_sessions: 12, payment_status: 'Ödendi' }]);
      ...
      <div className="max-w-6xl mx-auto">
        {members.some(m => m.remaining_sessions <= 2) && (
          <div className="mb-8 bg-orange-500/10 border border-orange-500/30 p-4 rounded-3xl animate-pulse flex items-center gap-3">
            <AlertTriangle className="text-orange-500" size={20}/><span className="text-[10px] font-black uppercase tracking-widest text-orange-200">Kritik Seanslar Mevcut!</span>
          </div>
        )}
        <header className="mb-12 text-center border-b border-white/5 pb-8">
          <h1 className="text-6xl font-black text-green-400 italic uppercase tracking-tighter drop-shadow-[0_0_20px_rgba(34,197,94,0.3)]">ONE TO ONE</h1>
        ...
        <section className="mb-12 text-left px-2">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-600 mb-6 flex items-center gap-2"><Clock className="text-green-500" size={16}/> Günün Randevuları ({format(new Date(), 'd MMMM', { locale: tr })})</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
                {todayAppointments.length > 0 ? todayAppointments.map((app) => (
                    <div key={app.id} className="bg-white/5 border border-white/5 p-6 rounded-[2.5rem] shadow-xl text-left">
                        <div className="flex justify-between items-start mb-4"><span className="font-black text-green-400 text-lg uppercase">{app.member_id}</span><span className="bg-green-500 text-black text-[10px] font-black px-3 py-1 rounded-full">{app.training_time}</span></div>
                        <p className="text-[10px] text-gray-400 italic leading-relaxed">{app.workout_program || 'Not yok'}</p>
                    </div>
                )) : <div className="col-span-full py-10 border border-dashed border-white/10 rounded-[2.5rem] text-center text-gray-600 font-black uppercase text-[10px]">Randevu yok.</div>}
            </div>
        </section>
        ...
        <section className="mb-12 bg-white/5 p-8 rounded-[3rem] border border-white/5 text-left shadow-2xl">
          <form onSubmit={handleAddMember} className="flex flex-col md:flex-row gap-4">
            <input type="text" placeholder="Yeni Üye" value={fullName} onChange={(e) => setFullName(e.target.value)} className="flex-1 bg-black/40 border border-white/10 rounded-2xl px-6 py-4 outline-none font-bold" />
            ...
            <div key={member.id} onClick={() => showProfile(member)} className={`group bg-white/5 p-8 rounded-[3rem] border transition-all flex justify-between items-center cursor-pointer shadow-xl ${member.remaining_sessions <= 2 ? 'border-orange-500/50 bg-orange-500/5' : 'border-white/5 hover:border-green-500/30'}`}>
              <div className="flex items-center gap-6">
                ...
                <div className="text-left">
                  <h3 className="font-bold uppercase text-2xl mb-1">{member.id}</h3>
              ...
              <div className="flex items-center gap-8">
                ...
                <div className="flex flex-col gap-2"><button onClick={(e) => updateSession(e, member.id, member.remaining_sessions + 1)} className="w-12 h-12 border border-white/10 rounded-2xl font-black flex items-center justify-center hover:bg-green-500 transition-all"><Plus/></button><button onClick={(e) => updateSession(e, member.id, member.remaining_sessions - 1)} className="w-12 h-12 border border-white/10 rounded-2xl font-black flex items-center justify-center hover:bg-red-500 transition-all"><Minus/></button></div>
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="mb-10 flex flex-col md:flex-row justify-between items-center md:items-end gap-10 border-b border-white/5 pb-10 text-left w-full">
                  <div className="text-left w-full">
                    ...
                    <div className="flex flex-wrap gap-5 justify-start text-left">
                      <div className="flex flex-col items-center"><input type="text" value={localGoal} onChange={(e) => setLocalGoal(e.target.value)} onBlur={() => saveField('goal_weight', parseFloat(localGoal) || 0)} className="bg-white/5 border border-white/10 rounded-2xl px-5 py-2 text-sm text-yellow-500 w-24 outline-none font-bold text-center" /><span className="text-[9px] text-gray-600 font-black uppercase mt-2">Hedef</span></div>
                ...
                <div className="space-y-8 text-left">
                  ...
                  {selectedDate && (
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6 animate-in slide-in-from-bottom duration-500 shadow-inner text-left">
                        <div className="flex justify-between items-center"><p className="text-green-400 font-black uppercase text-xs tracking-widest text-left">📅 {format(selectedDate, 'd MMMM EEEE', { locale: tr })}</p></div>
                        <input type="time" value={trainingTime} onChange={(e) => setTrainingTime(e.target.value)} className="w-full bg-black border border-white/10 rounded-2xl px-6 py-4 text-xl font-black text-green-400 outline-none" /><textarea value={workoutProgram} onChange={(e) => setWorkoutProgram(e.target.value)} className="w-full bg-black border border-white/10 rounded-3xl p-6 h-40 text-sm outline-none resize-none" placeholder="Program..."></textarea><button onClick={saveAppointment} className="w-full bg-green-500 text-black font-black py-4 rounded-[1.5rem] uppercase text-[10px] tracking-widest shadow-xl">Kaydet</button>
                    </div>
                  )}
                </div>
                <div className="space-y-8 text-left">
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/5 shadow-inner h-64">
                    ...
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-4 text-left shadow-inner">
                      <h3 className="text-[10px] font-black uppercase text-green-400 flex items-center gap-2 px-2"><Apple size={14}/> Beslenme Düzenle</h3>
                      <div className="grid grid-cols-2 gap-4 text-left">
                        <div className="space-y-1"><p className="text-[9px] text-gray-500 font-black uppercase ml-1">Kahvaltı</p><textarea onBlur={(e) => saveDietPlan('breakfast', e.target.value)} defaultValue={dietPlan?.breakfast} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs outline-none h-20 resize-none italic"/></div>
                        <div className="space-y-1"><p className="text-[9px] text-gray-500 font-black uppercase ml-1">Öğle</p><textarea onBlur={(e) => saveDietPlan('lunch', e.target.value)} defaultValue={dietPlan?.lunch} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs outline-none h-20 resize-none italic"/></div>
                        <div className="space-y-1"><p className="text-[9px] text-gray-500 font-black uppercase ml-1">Akşam</p><textarea onBlur={(e) => saveDietPlan('dinner', e.target.value)} defaultValue={dietPlan?.dinner} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs outline-none h-20 resize-none italic"/></div>
                        <div className="space-y-1"><p className="text-[9px] text-gray-500 font-black uppercase ml-1">Snack</p><textarea onBlur={(e) => saveDietPlan('snacks', e.target.value)} defaultValue={dietPlan?.snacks} className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs outline-none h-20 resize-none italic"/></div>
                      </div>
                    </div>
                    <textarea value={localNotes} onChange={(e) => setLocalNotes(e.target.value)} onBlur={() => saveField('notes', localNotes)} className="w-full h-40 bg-black/60 border border-white/5 rounded-[2.5rem] p-7 text-sm outline-none shadow-inner" placeholder="Hoca notları..."></textarea>
                </div>
              </div>
            </div>
          </div>
