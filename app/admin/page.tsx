...
export default function AdminDashboard() {
  ...
  async function handleAddMember(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('members').insert([{ id: fullName, slug: slugify(fullName), password: newPass, remaining_sessions: 12, payment_status: 'Ödendi' }]);
  ...
  async function addMeasurement() {
    ...
    const { error } = await supabase.from('body_measurements').insert([{
      ...
      chest: parseFloat(mChest) || 0,
      arm: parseFloat(mArm) || 0,
    ...
    <main className="min-h-screen bg-[#050505] text-white p-4 md:p-8 font-sans selection:bg-green-500/30 overflow-x-hidden text-left">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-8">
            ...
            <button onClick={handleLogout} className="bg-white/5 p-4 rounded-2xl hover:bg-red-500/20 transition-all group">
        ...
        <section className="mb-12 text-left">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-600 mb-6 flex items-center gap-2 px-2 text-left"><Clock className="text-green-500" size={16}/> Günün Ajandası ({format(new Date(), 'd MMMM', { locale: tr })})</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
                {todayAppointments.length > 0 ? todayAppointments.map((app) => (
                    <div key={app.id} className="bg-white/5 border border-white/5 p-6 rounded-[2.5rem] shadow-xl text-left">
                        ...
                        <p className="text-[10px] text-gray-400 italic leading-relaxed">{app.workout_program || 'Program yok'}</p>
                    </div>
        ...
        <section className="mb-12 bg-white/5 p-8 rounded-[3rem] border border-white/5 text-left shadow-2xl">
          <form onSubmit={handleAddMember} className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative md:w-48">
              ...
              <input type="text" placeholder="Şifre" value={newPass} onChange={(e) => setNewPass(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 pl-10 pr-4 outline-none font-bold text-xs" />
            </div>
            <button type="submit" className="bg-green-500 text-black font-black px-12 py-4 rounded-2xl uppercase text-sm active:scale-95 transition-all">Sisteme Ekle</button>
          </form>
        </section>
          ...
          {members.map((member) => (
            <div key={member.id} onClick={() => showProfile(member)} className={`group bg-white/5 p-8 rounded-[3rem] border transition-all flex justify-between items-center cursor-pointer shadow-xl ${member.remaining_sessions <= 2 ? 'border-orange-500/50 bg-orange-500/5' : 'border-white/5 hover:border-green-500/30'}`}>
              <div className="flex items-center gap-6 text-left">
                <button onClick={(e) => { e.stopPropagation(); if (window.confirm('Üye silinsin mi?')) { supabase.from('members').delete().eq('id', member.id).then(() => fetchData()); } }} className="opacity-10 group-hover:opacity-100 hover:text-red-500 transition-all"><Trash2 size={22}/></button>
                <div className="text-left">
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`${window.location.origin}/${member.slug}`); alert("Link kopyalandı!"); }} className="text-gray-500 hover:text-blue-400 transition-colors"><Copy size={14}/></button>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-8 text-left">
                ...
                <div className="flex flex-col gap-2">
                    <button onClick={(e) => updateSession(e, member.id, member.remaining_sessions + 1)} className="w-10 h-10 border border-white/10 rounded-xl font-black flex items-center justify-center hover:bg-green-500 transition-all"><Plus size={18}/></button>
                    <button onClick={(e) => updateSession(e, member.id, member.remaining_sessions - 1)} className="w-10 h-10 border border-white/10 rounded-xl font-black flex items-center justify-center hover:bg-red-500 transition-all"><Minus size={18}/></button>
                </div>
              </div>
            </div>
          ))}
        ...
        {selectedMember && (
          <div className="fixed inset-0 bg-black/95 z-[100] overflow-y-auto p-4 md:p-10 flex items-start justify-center">
            <div className="bg-[#080808] border border-white/10 rounded-[3.5rem] w-full max-w-6xl relative p-8 md:p-12 shadow-2xl flex flex-col my-auto text-left">
              ...
              <div className="mb-10 flex flex-col md:flex-row justify-between items-center md:items-end gap-10 border-b border-white/5 pb-10 text-left w-full">
                  <div className="text-left w-full">
                    <h2 className="text-5xl font-black text-green-400 uppercase leading-none mb-6">{selectedMember.id}</h2>
                    <div className="flex flex-wrap gap-5 justify-start text-left">
                      <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase text-gray-600">Giriş Şifresi:</span>
                        <input type="text" defaultValue={selectedMember.password} onBlur={(e) => saveField('password', e.target.value)} className="bg-transparent border-none outline-none text-xs font-bold text-blue-400 w-24" />
                        <button onClick={() => {
                        ...
                        }} className="text-green-500 hover:scale-110 transition-transform"><Send size={16}/></button>
                      </div>
              ...
              <div className="grid lg:grid-cols-2 gap-12 text-left">
                ...
                <div className="space-y-8 text-left">
                  ...
                  {selectedDate && (
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6 animate-in slide-in-from-bottom duration-500 shadow-inner text-left text-left">
                        <div className="flex justify-between items-center"><p className="text-green-400 font-black uppercase text-xs tracking-widest">📅 {format(selectedDate, 'd MMMM EEEE', { locale: tr })}</p>{appointments.some(a => isSameDay(new Date(a.training_date), selectedDate)) && (<button onClick={() => { if (window.confirm('Randevu silinsin mi?')) { supabase.from('appointments').delete().eq('member_id', selectedMember.id).eq('training_date', format(selectedDate, 'yyyy-MM-dd')).then(() => showProfile(selectedMember)); } }} className="text-red-500/50 hover:text-red-500 text-[10px] font-black uppercase flex items-center gap-1 transition-all"><Trash size={14}/> Sil</button>)}</div>
                        <input type="time" value={trainingTime} onChange={(e) => setTrainingTime(e.target.value)} className="w-full bg-black border border-white/10 rounded-2xl px-6 py-4 text-xl font-black text-green-400 outline-none text-left" /><textarea value={workoutProgram} onChange={(e) => setWorkoutProgram(e.target.value)} className="w-full bg-black border border-white/10 rounded-3xl p-6 h-40 text-sm outline-none resize-none text-left leading-relaxed" placeholder="Bugünkü antrenman programı..."></textarea><button onClick={saveAppointment} className="w-full bg-green-500 text-black font-black py-4 rounded-[1.5rem] uppercase text-[10px] tracking-widest active:scale-95 transition-all shadow-xl shadow-green-500/10">Randevuyu Kaydet</button>
                    </div>
                  )}
                  <textarea value={localNotes} onChange={(e) => setLocalNotes(e.target.value)} onBlur={() => saveField('notes', localNotes)} className="w-full h-64 bg-black/60 border border-white/5 rounded-[2.5rem] p-8 text-sm outline-none resize-none shadow-inner italic text-left leading-relaxed" placeholder="Genel antrenör notları..."></textarea>
                </div>
                ...
                <div className="space-y-8 text-left">
                    ...
                    <form onSubmit={addWeight} className="flex flex-col gap-4 bg-black/60 p-6 rounded-[2.5rem] border border-white/5 text-left text-left"><p className="text-[10px] text-gray-600 font-black uppercase ml-2 tracking-widest">Kilo Girişi</p><input type="number" step="0.1" value={newWeight} onChange={(e) => setNewWeight(e.target.value)} className="bg-transparent px-4 py-2 text-2xl font-black border-b border-white/5 outline-none text-left" placeholder="00.0" /><button type="submit" className="bg-green-500 text-black font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest shadow-lg">Kilo Ekle</button></form>
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-6 shadow-inner text-left">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-left">
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1">Omuz</p><input type="number" value={mShoulder} onChange={(e) => setMShoulder(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1">Göğüs</p><input type="number" value={mChest} onChange={(e) => setMChest(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1">Kol</p><input type="number" value={mArm} onChange={(e) => setMArm(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none" placeholder="0"/></div>
                        <div className="flex flex-col gap-1 text-left text-left"><p className="text-[8px] text-gray-500 font-black uppercase ml-1">Bel</p><input type="number" value={mWaist} onChange={(e) => setMWaist(e.target.value)} className="bg-black/60 border border-white/5 rounded-xl p-3 text-xs outline-none" placeholder="0"/></div>
                      </div>
                      <button onClick={addMeasurement} className="w-full bg-blue-500 text-white font-black py-3 rounded-2xl uppercase text-[9px] active:scale-95 transition-all shadow-lg shadow-blue-500/10">Ölçüleri Kaydet</button>
                      {measurements.length > 0 && (
                        <div className="mt-4 border-t border-white/5 pt-4 space-y-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar text-left text-left">
                          {measurements.map((m, i) => (
                            <div key={i} className="flex justify-between items-center text-[9px] bg-black/40 p-2 rounded-lg border border-white/5 group text-left">
                              <span className="text-gray-500">{format(new Date(m.recorded_at), 'd MMM')}</span>
                              <span className="text-blue-400 font-black uppercase">O:{m.shoulder} G:{m.chest} K:{m.arm} B:{m.waist}</span>
                              <button onClick={() => { if (window.confirm('Ölçüm silinsin mi?')) { supabase.from('body_measurements').delete().eq('id', m.id).then(() => showProfile(selectedMember)); } }} className="opacity-0 group-hover:opacity-100 text-red-500 transition-all"><Trash2 size={10}/></button>
                            </div>
                          ))}
                        </div>
                      )}
                    </div>
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 space-y-4 shadow-inner text-left">
